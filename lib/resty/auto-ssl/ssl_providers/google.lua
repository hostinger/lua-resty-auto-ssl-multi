...
function _M.issue_cert(auto_ssl_instance, domain)
  ...
  local result, err = shell_execute({
    ...
    "--domain", domain,
    "--challenge", "http-01",
    "--config", base_dir .. "/google/config",
    "--hook", lua_root .. "/bin/resty-auto-ssl/google_hooks",
  })
  ...
  local cert, get_cert_err = storage:get_cert(domain, "google")
...
function _M.cleanup(auto_ssl_instance, domain)
  ...
  local dir = auto_ssl_instance:get("dir") .. "/google/certs/" .. domain
